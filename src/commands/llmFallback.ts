 ...
 *
 * GPT-4o-mini fallback for voice commands that the local grammar cannot parse.
 *
 * Uses OpenAI function-calling so the model is forced to return a value that
 ...
 *
 * Environment:
 *   VITE_OPENAI_API_KEY — client-side key (Vite exposes VITE_* vars to the
...
// ─── JSON Schema for the function definition ──────────────────────────────────

const PARSE_COMMAND_FUNCTION: OpenAI.Chat.Completions.ChatCompletionCreateParams.Function =
  {
...
function getClient(): OpenAI {
  ...
  if (!apiKey) {
    throw createVoiceError('LLM_FALLBACK_ERROR', {
      message: 'VITE_OPENAI_API_KEY is not set — cannot call the LLM fallback',
    })
  }
...
/**
 * Call GPT-4o-mini with function-calling to parse `transcript` into a
 * `ShapeCommand`.
 *
...
export async function llmFallback(transcript: string): Promise<ShapeCommand | null> {
  ...
  try {
    const response = await client.chat.completions.create({
      ...
      functions: [PARSE_COMMAND_FUNCTION],
      function_call: { name: 'parse_voice_command' },
      temperature: 0,
    })
